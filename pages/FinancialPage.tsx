    ...
    <div className="p-6 space-y-6 pb-20 animate-in fade-in duration-300">
      <header className="flex flex-col xl:flex-row xl:items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <Plus size={16} /> Inserir Receita
          </button>
          <button
            onClick={() => openEntryModal('DESPESA')}
            className="px-4 py-2.5 rounded-xl bg-red-600 text-white text-xs font-black uppercase tracking-widest hover:bg-red-700 flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <div className="bg-white border rounded-2xl overflow-hidden">
          <div className="p-4 border-b bg-gray-50 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-black text-red-600">{filteredPendingClients.length} cliente(s)</span>
            </div>
          </div>
          <div className="p-4 border-b bg-white grid grid-cols-1 md:grid-cols-2 gap-3">
            <input
              type="text"
              value={pendingSearch}
              onChange={(e) => setPendingSearch(e.target.value)}
              placeholder="Filtrar por aluno, matrícula, responsável ou plano..."
              className="w-full px-3 py-2.5 rounded-xl bg-gray-50 border-2 border-transparent focus:border-indigo-500 outline-none text-sm font-bold"
            />
            <select
              value={pendingTypeFilter}
              onChange={(e) => setPendingTypeFilter(e.target.value as 'ALL' | 'ALUNO' | 'COLABORADOR')}
              className="w-full px-3 py-2.5 rounded-xl bg-gray-50 border-2 border-transparent focus:border-indigo-500 outline-none text-xs font-black uppercase tracking-widest"
            >
        ...
        <div className="bg-white border rounded-2xl overflow-hidden">
          ...
          <div className="p-4 border-b bg-white grid grid-cols-1 md:grid-cols-3 gap-3">
            <input
              type="text"
            ...
            {reminderTimeFilter === 'DATE' ? (
              <input
                ...
                className="w-full px-3 py-2.5 rounded-xl bg-gray-50 border-2 border-transparent focus:border-indigo-500 outline-none text-xs font-black"
              />
            ) : (
          ...
          <div className="overflow-x-auto max-h-[360px]">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-[10px] font-black uppercase tracking-widest text-gray-400">
                <tr>
                  <th className="px-4 py-3 text-left">Data Lembrete</th>
              ...
              <tbody className="divide-y">
                {reminders.length === 0 ? (
                  ...
                  <tr key={`rem_${tx.id}`}>
                    <td className="px-4 py-3 font-bold text-gray-700">{formatDateBr(tx.reminderDate)}</td>
                    <td className="px-4 py-3 text-gray-800 font-bold">{tx.description}</td>
                    <td className="px-4 py-3 text-gray-600 font-bold">{tx.monthReference || '-'}</td>
                    <td className="px-4 py-3 text-right font-black text-red-600">R$ {tx.amount.toFixed(2)}</td>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {filteredLaunchTransactions.length === 0 ? (
                ...
                <tr key={tx.id}>
                  <td className="px-4 py-3 font-bold text-gray-700">{formatDateBr(tx.date)} {tx.time}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-gray-700 font-bold">{tx.category}</td>
                  <td className="px-4 py-3 text-gray-800 font-bold">{tx.description}</td>
                  <td className="px-4 py-3 text-right font-bold">{tx.quantity}</td>
                  <td className="px-4 py-3 text-right font-bold">R$ {tx.unitPrice.toFixed(2)}</td>
                  <td className={`px-4 py-3 text-right font-black ${tx.type === 'RECEITA' ? 'text-emerald-700' : 'text-red-700'}`}>
                    {tx.type === 'RECEITA' ? '+' : '-'} R$ {tx.amount.toFixed(2)}
                  </td>
                  <td className="px-4 py-3 text-gray-600 font-bold">{tx.dueDate ? formatDateBr(tx.dueDate) : '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
const MetricCard = ({ label, value, sub, icon, color }: { label: string; value: number; sub: string; icon: React.ReactElement; color: string }) => (
  <div className={`${color} rounded-2xl p-6 text-white flex items-center justify-between`}>
    <div>
      <p className="text-[10px] font-black uppercase tracking-widest opacity-70">{label}</p>
      <p className="text-3xl font-black">R$ {Number(value || 0).toFixed(2)}</p>
      <p className="text-[10px] font-bold uppercase tracking-widest opacity-70">{sub}</p>
    </div>
